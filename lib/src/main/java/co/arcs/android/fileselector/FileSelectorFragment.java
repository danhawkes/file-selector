...
public class FileSelectorFragment extends Fragment {
	...
	public static FileSelectorFragment newInstance(String initialDirectory,
			boolean allowNavAboveInitial, SelectionMode selectionMode, boolean showFiles,
			boolean showHidden) {
		...
		args.putString(ARG_SELECTION_TYPE, (selectionMode == SelectionMode.FILE) ? TYPE_FILE
				: TYPE_DIRECTORY);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.initialDirectory = new File(args.getString(ARG_STR_INITIAL_DIRECTORY, Environment
				.getExternalStorageDirectory().getPath()));
		if (!initialDirectory.isDirectory()) {
			throw new InvalidParameterException(String.format(Locale.US, "'%s' is not a directory",
					initialDirectory));
		}
		this.allowNavigateUpAboveInitial = args.getBoolean(
				ARG_BOOL_ALLOW_UP_NAVIGATION_ABOVE_INITIAL_DIRECTORY, false);
		this.selectionType = (args.getString(ARG_SELECTION_TYPE, TYPE_FILE).equals(TYPE_FILE)) ? SelectionMode.FILE
